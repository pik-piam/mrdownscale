...
#'
#' @param xInput magpie object to fade to, usually model projections
#' @param xTarget magpie object to fade from, usually historical data, this argument is called "target"
...
#' @author Pascal Sauer
toolHarmonizeOffset <- function(xInput, xTarget, harmonizationPeriod) {
  "!# @monitor mip::harmonize"

  .df <- function(xInput) {
    df <- magclass::as.data.frame(xInput, rev = 3)
  ...
  getSets(out)[1] <- "id"
  spatElems <- getItems(xInput, dim = 1.1, full = TRUE)
  names(spatElems) <- getItems(xInput, dim = "id", full = TRUE)
